...
class PluginFinderSearch():
    ...
    def directory(self):
        ...
        if not Path(directoryJsonPath).exists() or datetime.fromtimestamp(
            os.path.getmtime(str(directoryJsonPath))
        ) < datetime.now() - timedelta(days=1):
            self.updateDirectory()
        return json.load(open(directoryJsonPath))
    ...
    def pluginData(self, pluginId=str):
        ...
        # If the file is missing or old, update it.
        if not Path(
            self.paths.pluginDataCachePath(pluginId)
        ).exists() or datetime.fromtimestamp(
            os.path.getmtime(str(self.paths.pluginDataCachePath(pluginId)))
    ...
    def pagedPluginData(self, searchTerms=str, installed=False, page=int, pageSize=int):
        ...
        qInfo(
            f"Searching directory, terms = {str(searchTerms)} installed = {str(installed)}"
        )
        ...
        results = [
            self.pluginData(str(item["Identifier"]))
    ...
    def getGithubReleaseJson(self, author=str, repo=str, tag=str):
        ...
        if Path(cacheName).exists() and datetime.fromtimestamp(
            os.path.getmtime(str(cacheName))
        ) > datetime.now() - timedelta(days=1):
        ...
        if "assets" in releaseJson:
            ...
            for asset in assets:
                if asset["browser_download_url"].endswith(zipName):
                    return int(asset["download_count"])
        return None
    ...
    def getTotalGithubDownloadCount(self, data=PluginData):
        ...
        if plugin in counters:
            ...
            counters[plugin] = { }
            counters[plugin]["Updated"] = datetime.now().isoformat()
        ...
        #try:
            for version in data.versions():
                if match := re.match(self.downloadUrlRegex, version.downloadUrl()):
    ...
    def loadCounters(self):
        if self.paths.counterJsonPath().exists():
            return json.load(open(self.paths.counterJsonPath()))
        return {}
    ...
    def tryCreateInstallCount(self, pluginId=str):
        ...
        createUrl = f"https://api.countapi.xyz/create?key={pluginId}&namespace=pluginfinder&value=0"
        try:
    ...
    def refreshPluginCount(self, pluginId=str):
        ...
        if pluginId not in counters:
            counters[pluginId] = { }
        counters[pluginId]["Updated"] = datetime.now().isoformat()
        counters[pluginId]["Downloads"] = "?"
        self.tryCreateInstallCount(pluginId)
        url = f"https://api.countapi.xyz/get/pluginfinder/{str(pluginId)}"
        try:
